...
const FROM = process.env.EMAIL_FROM || "noreply@safemind.com.br";
...
async function sendEmail({ to, subject, html }: { to: string; subject: string; html: string }) {
  ...
  if (transport) {
    const from = process.env.GMAIL_USER ? `SafeMind <${process.env.GMAIL_USER}>` : FROM;
...
export async function sendInviteEmail({
  ...
  return sendEmail({
    to,
    subject: `Você foi convidado para o SafeMind na ${companyName}`,
    html: `
      <h2>Olá, ${name}!</h2>
      <p>Você foi convidado para acessar a plataforma SafeMind NR-1 da empresa <strong>${companyName}</strong>.</p>
      <p>Clique no link abaixo para criar sua senha. O link expira em 7 dias.</p>
      <a href="${link}" style="background:#7C3AED;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin:16px 0;">Aceitar Convite</a>
...
export async function sendSurveyOpenEmail({
  ...
  return sendEmail({
    ...
    html: `
      ...
      <a href="${surveyLink}" style="background:#7C3AED;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin:16px 0;">Responder Pesquisa</a>
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#111">
        <div style="background:#7C3AED;border-radius:12px 12px 0 0;padding:24px;text-align:center">
          <h1 style="color:white;margin:0;font-size:20px">SafeMind NR-01</h1>
          <p style="color:#DDD6FE;margin:4px 0 0;font-size:13px">Pesquisa de Clima Organizacional</p>
        </div>
        <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-top:none;border-radius:0 0 12px 12px;padding:28px">
          ...
          <div style="text-align:center;margin:24px 0">
            <a href="${surveyLink}" style="background:#7C3AED;color:white;padding:14px 32px;border-radius:8px;text-decoration:none;display:inline-block;font-weight:600;font-size:15px">
              Responder Pesquisa
            </a>
          </div>
          <div style="background:#EDE9FE;border-radius:8px;padding:12px 16px;font-size:12px;color:#5B21B6">
          ...
          <p style="margin-top:20px;font-size:12px;color:#9CA3AF;text-align:center">
            Se não esperava este email, ignore-o. Ele foi enviado pelo SafeMind em nome de ${companyName}.
          </p>
        </div>
      </div>
    `,
...
export async function sendSurveyReminderEmail({
  ...
  return sendEmail({
    ...
    html: `
      ...
      <a href="${surveyLink}" style="background:#7C3AED;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin:16px 0;">Responder Agora</a>
    `,
  });
}
...
export async function sendSurveyClosedEmail({
  ...
  return sendEmail({
    ...
    html: `
      ...
      <a href="${resultsLink}" style="background:#7C3AED;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin:16px 0;">Ver Resultados</a>
    `,
  });
}
...
export async function sendComplaintReceivedEmail({
  ...
  return sendEmail({
    ...
    html: `
      ...
      <a href="${complaintsLink}" style="background:#7C3AED;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;margin:16px 0;">Ver Denúncia</a>
